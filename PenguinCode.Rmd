...
#calculates the r squared for each species of penguin
r_squared_by_group <- culmen_data %>%
  group_by(species) %>%
  summarise(r_squared = cor(culmen_length_mm, culmen_depth_mm)^2)
print(r_squared_by_group)

combined_stats <- (summary(all_penguin_model) | r_squared_by_group)
combined_stats
```
...
overall_lm <- plot_scatter_lm(culmen_data, culmen_length_mm, culmen_depth_mm,"Culmen Length (mm)", "Culmen Depth (mm)", species, species_colours,"Adelie","Chinstrap","Gentoo")
